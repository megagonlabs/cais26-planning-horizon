"""
Retrieve similar in-context examples for GraphQ train/test samples.

Dataset-specific configuration for GraphQ retrieval using shared utilities.

Usage:
    python data/atomic_kbqa/graphq/scripts/retrieve_graphq_examples.py --num-candidates 50
    python data/atomic_kbqa/graphq/scripts/retrieve_graphq_examples.py --num-candidates 10 --batch-size 128
"""
...
def main(args):
    """Main retrieval workflow for GraphQ."""

    # Auto-generate output paths if not provided
    if args.train_output is None:
        stem = args.train_input.stem  # e.g., "graphq_train.v1"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Retrieve similar in-context examples for GraphQ samples"
    )
    parser.add_argument(
        "--heldout-pool",
        dest="heldout_pool",
        type=Path,
        default=Path("data/atomic_kbqa/graphq/processed/graphq_train_heldout_pool.v1.json"),
    ...
    parser.add_argument(
        ...
        dest="train_input",
        type=Path,
        default=Path("data/atomic_kbqa/graphq/processed/graphq_train.v1.json"),
    ...
    parser.add_argument(
        ...
        dest="test_input",
        type=Path,
        default=Path("data/atomic_kbqa/graphq/processed/graphq_test.v1.json"),
