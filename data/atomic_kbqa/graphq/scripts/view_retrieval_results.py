"""
Streamlit app to view GrailQA retrieval results.
...
Usage:
    streamlit run data/atomic_kbqa/grailqa/scripts/view_retrieval_results.py
"""
...
def main():
    st.title("GrailQA Retrieval Results Viewer")

    # File paths
    data_dir = Path(__file__).parent.parent / "processed"
    train_file = data_dir / "grailqa_train.v1.50nn.json"
    test_file = data_dir / "grailqa_test.v1.50nn.json"
    ...
    try:
        ...
        st.code(
            "uv run python data/atomic_kbqa/grailqa/scripts/retrieve_grailqa_examples.py --num-candidates 50",
            language="bash",
        )
        st.stop()
    ...
    # Level filter (GrailQA specific)
    levels = sorted(set(ex["metadata"].get("level", "unknown") for ex in all_data))
    selected_levels = st.sidebar.multiselect("Difficulty Level", levels, default=levels)
    ...
    for i, example in enumerate(all_data):
        ...
        # Level filter
        if example["metadata"].get("level", "unknown") not in selected_levels:
            continue
    ...
    for idx in filtered_indices:
        ...
        level = example["metadata"].get("level", "unknown")
        option_text = (
            f"{example['id']} [{split}] [{level}] "
