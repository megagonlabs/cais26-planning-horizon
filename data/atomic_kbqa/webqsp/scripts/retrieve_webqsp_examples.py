"""
Retrieve similar in-context examples for WebQSP train/test samples.

Dataset-specific configuration for WebQSP retrieval using shared utilities.

Usage:
    python data/atomic_kbqa/webqsp/scripts/retrieve_webqsp_examples.py --num-candidates 50
    python data/atomic_kbqa/webqsp/scripts/retrieve_webqsp_examples.py --num-candidates 10 --batch-size 128
"""
...
def main(args):
    """Main retrieval workflow for WebQSP."""

    # Auto-generate output paths if not provided
    if args.train_output is None:
        stem = args.train_input.stem  # e.g., "webqsp_train.v1"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Retrieve similar in-context examples for WebQSP samples"
    )
    parser.add_argument(
        "--heldout-pool",
        dest="heldout_pool",
        type=Path,
        default=Path("data/atomic_kbqa/webqsp/processed/WebQSP_train_heldout_pool.v1.json"),
    ...
    parser.add_argument(
        ...
        dest="train_input",
        type=Path,
        default=Path("data/atomic_kbqa/webqsp/processed/webqsp_train.v1.json"),
    ...
    parser.add_argument(
        ...
        dest="test_input",
        type=Path,
        default=Path("data/atomic_kbqa/webqsp/processed/webqsp_test.v1.json"),
